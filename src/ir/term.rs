...
impl<'a, 'b> Term<'a, 'b, Core> {
  ...
  pub fn infer(
    ...
    match self {
      ...
      Term::Ann(x_old, t_old) => {
        ...
        let t_val = Val::eval(t_old, env, ar)?;
      ...
      Term::Let(info, v_old, x_old) => {
        ...
        let v_val = Val::eval(v_old, env, ar)?;
      ...
      Term::Pi(info, t_old, u_old) => {
        ...
        let ctx_ext = ctx.extend(info, Val::eval(t_old, env, ar)?, ar);
      ...
      Term::App(f_old, x_old, dot) => {
        ...
        let x_new = x_old.check(*t_val, ctx, env, ar)?;
        Ok(((Term::App(ar.term(f_new), ar.term(x_new), *dot)), Val::apply(u_val, Val::eval(x_old, env, ar)?, ar)?))
      }
      ...
      Term::Var(ix) => Term::present_named_var(*ix, None, ctx, env, ar),
      Term::Proj(n, Term::Var(ix)) => Term::present_named_var(*ix, Some(*n), ctx, env, ar),
      Term::Proj(n, x_old) => {
        let (x_new, x_type) = x_old.infer(ctx, env, ar)?;
        Term::present_named_proj(*n, x_old, x_new, x_type, ctx, env, ar)
      }
      ...
      Term::Tup(bs_old) => {
        ...
        if bs_old.len() == us_val.len() {
          ...
          for (i, (info, b_old)) in bs_old.iter().enumerate() {
            ...
            bs_new[i] = (info, b_new);
            let b_val = Val::eval(b_old, &env_ext, ar)?;
  ...
  pub fn present_named_var(
    ...
    match proj {
      ...
      Some(n) => {
        let us_val = Term::as_sig(t_val, |t_val| TypeError::sig_expected(ar.term(Term::Var(ix)), t_val, ctx, env, ar))?;
        let i =
          us_val.len().checked_sub(n + 1).ok_or_else(|| TypeError::sig_proj(n, Val::Sig(us_val), ctx, env, ar))?;
        let (info, u_val) = &us_val[i];
        let u_val = Val::apply(u_val, Val::eval(&Term::Init(n + 1, ar.term(Term::Var(ix))), env, ar)?, ar)?;
  ...
  pub fn present_named_proj(
    ...
    let (info, u_val) = &us_val[i];
    let u_val = Val::apply(u_val, Val::eval(&Term::Init(n + 1, x_old), env, ar)?, ar)?;
