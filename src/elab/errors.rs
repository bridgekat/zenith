...
impl<'a> TypeError<'a> {
  ...
  pub fn type_expected<'b>(term: &'a Term<'a>, ty: ir::Val<'b, 'a>, len: usize, ar: &'a Arena) -> Self {
    match ar.val(ty).quote(len, ar) {
      Ok(ty) => Self::TypeExpected { term, ty },
      Err(err) => err.into(),
    }
  }

  pub fn pi_expected<'b>(term: &'a Term<'a>, ty: ir::Val<'b, 'a>, len: usize, ar: &'a Arena) -> Self {
    match ar.val(ty).quote(len, ar) {
  ...
  pub fn sig_expected<'b>(term: &'a Term<'a>, ty: ir::Val<'b, 'a>, len: usize, ar: &'a Arena) -> Self {
    match ar.val(ty).quote(len, ar) {
  ...
  pub fn pi_ann_expected<'b>(ty: ir::Val<'b, 'a>, len: usize, ar: &'a Arena) -> Self {
    match ar.val(ty).quote(len, ar) {
  ...
  pub fn sig_ann_expected<'b>(ty: ir::Val<'b, 'a>, len: usize, ar: &'a Arena) -> Self {
    match ar.val(ty).quote(len, ar) {
  ...
  pub fn type_mismatch<'b>(
    ...
    match ar.val(ty).quote(len, ar) {
      Ok(ty) => match ar.val(expect).quote(len, ar) {
