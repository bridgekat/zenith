...
use crate::ir::{Bound, Clos, Core, Field, Name, Named, Stack, Term, TypeError, Val};
...
impl<'a, 'b> Term<'a, 'b, Named> {
  ...
  pub fn infer(
    ...
    match self {
      ...
      Term::Ann(x_old, t_old) => {
        ...
        let _ = Term::as_univ(t_type, |t_type| TypeError::type_expected(t_old, t_type, ctx, env, ar))?;
        let t_val = Val::eval(&t_new, env, ar)?;
      ...
      Term::Let(info, v_old, x_old) => {
        ...
        let v_val = Val::eval(&v_new, env, ar)?;
      ...
      Term::Pi(info, t_old, u_old) => {
        let (t_new, t_type) = t_old.infer(ctx, env, ar)?;
        let t_lvl = Term::as_univ(t_type, |t_type| TypeError::type_expected(t_old, t_type, ctx, env, ar))?;
        let ctx_ext = ctx.extend(info, Val::eval(&t_new, env, ar)?, ar);
        let env_ext = env.extend(info, Val::Free(env.len()), ar);
        let (u_new, u_type) = u_old.infer(&ctx_ext, &env_ext, ar)?;
        let u_lvl = Term::as_univ(u_type, |u_type| TypeError::type_expected(u_old, u_type, ctx, env, ar))?;
      ...
      Term::App(f_old, x_old, dot) => {
        ...
        let (t_val, u_val) = Term::as_pi(f_type, |f_type| TypeError::pi_expected(f_old, f_type, ctx, env, ar))?;
        let x_new = x_old.check(*t_val, ctx, env, ar)?;
        Ok(((Term::App(ar.term(f_new), ar.term(x_new), *dot)), Val::apply(u_val, Val::eval(&x_new, env, ar)?, ar)?))
      }
      ...
      Term::Init(n, x_old) => {
        ...
        let us_val = Term::as_sig(x_type, |x_type| TypeError::sig_expected(x_old, x_type, ctx, env, ar))?;
      ...
      Term::Proj(n, x_old) => {
        ...
        Ok((
          ...
          Val::apply(&us_val[i].1, Val::eval(&Term::Init(n + 1, ar.term(x_new)), env, ar)?, ar)?,
        ))
      }
      Term::NamedVar(name, _) => Term::resolve_named_var(name, ctx, env, ar),
      Term::NamedProj(name, x_old, _) => {
        let (x_new, x_type) = x_old.infer(ctx, env, ar)?;
        Term::resolve_named_proj(name, x_old, x_new, x_type, ctx, env, ar)
      }
    }
  }
  ...
  pub fn check(
    ...
    match self {
      ...
      Term::Fun(info, b_old) => {
        let (t_val, u_val) = Term::as_pi(t, |t| TypeError::pi_ann_expected(t, ctx, env, ar))?;
        let x_val = Val::Free(env.len());
        let ctx_ext = ctx.extend(info, *t_val, ar);
        let env_ext = env.extend(info, x_val, ar);
        let b_new = b_old.check(Val::apply(u_val, x_val, ar)?, &ctx_ext, &env_ext, ar)?;
        Ok(Term::Fun(info, ar.term(b_new)))
      }
      // The (∑ intro) and (extend) rules are used.
      // By pre-conditions, `t` is already known to have universe type.
      Term::Tup(bs_old) => {
        let us_val = Term::as_sig(t, |t| TypeError::sig_ann_expected(t, ctx, env, ar))?;
        if bs_old.len() == us_val.len() {
          ...
          for (i, (info, b_old)) in bs_old.iter().enumerate() {
            ...
            let b_new = b_old.check(Val::apply(u_val, a_val, ar)?, &ctx_ext, &env_ext, ar)?;
            bs_new[i] = (info, b_new);
            let b_val = Val::eval(&b_new, &env_ext, ar)?;
  ...
  pub fn resolve_named_var(
    ...
    let (ix, proj, x_type) = ctx.get_by_name(*name, env, ar).ok_or_else(|| ElabError::ctx_name(*name))?;
    match proj {
    ...
    match x_type {
      Val::Sig(us) => {
        for (n, (info, u)) in us.iter().rev().enumerate() {
          ...
          if info.name == *name {
            let t = Val::apply(u, Val::eval(&Term::Init(n + 1, ar.term(x_new)), env, ar)?, ar)?;
            return Ok((Term::Proj(n, ar.term(x_new)), t));
          }
        }
        Err(ElabError::sig_name(*name, x_old, x_type, ctx, env, ar))
      }
      _ => Err(ElabError::sig_expected(*name, x_old, x_type, ctx, env, ar)),
    }
  }
}
