...
namespace Server::LSP {
  ...
  #define from_to_simple(Type, ...) \
    void to_json(json& nlohmann_json_j, const Type& nlohmann_json_t) { NLOHMANN_JSON_EXPAND(NLOHMANN_JSON_PASTE(NLOHMANN_JSON_TO, __VA_ARGS__)) } \
    void from_json(const json& nlohmann_json_j, Type& nlohmann_json_t) { NLOHMANN_JSON_EXPAND(NLOHMANN_JSON_PASTE(NLOHMANN_JSON_FROM, __VA_ARGS__)) }
  ...
  #define from_to_enum(EnumType, ...) \
    void to_json(json& j, const EnumType& e) {                                     \
      ...
      auto it = std::find_if(std::begin(m), std::end(m),                           \
        [e](const std::pair<EnumType, json>& ej_pair) -> bool {                    \
    ...
    void from_json(const json& j, EnumType& e) {                                   \
      ...
      static const std::pair<EnumType, json> m[] = __VA_ARGS__;                    \
      auto it = std::find_if(std::begin(m), std::end(m),                           \
        [&j](const std::pair<EnumType, json>& ej_pair) -> bool {                   \
